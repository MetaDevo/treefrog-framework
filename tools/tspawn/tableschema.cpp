...
QMap<QString, QPair<QString, QString>> TableSchema::getParentTables() const
{
    ...
    query.prepare(
            "SELECT tc.table_schema, tc.constraint_name, tc.table_name, kcu.column_name, ccu.table_schema AS foreign_table_schema, ccu.table_name AS foreign_table_name, ccu.column_name AS foreign_column_name FROM information_schema.table_constraints AS tc JOIN information_schema.key_column_usage AS kcu ON tc.constraint_name = kcu.constraint_name JOIN information_schema.constraint_column_usage AS ccu ON ccu.constraint_name = tc.constraint_name WHERE constraint_type = 'FOREIGN KEY' AND tc.table_name='" +
            tablename + "'");
    query.exec();
    while (query.next()) {
        QPair<QString, QString> qp(query.value(5).toString(), query.value(6).toString());
        map[query.value(3).toString()] = qp;
    }
    return map;
}

QMap<QString, QPair<QString, QString>> TableSchema::getChildTables() const
{
    QMap<QString, QPair<QString, QString>> map;
    QSqlQuery query(QSqlDatabase::database());
    query.prepare(
            "SELECT tc.table_schema, tc.constraint_name, tc.table_name, kcu.column_name, ccu.table_schema AS foreign_table_schema, ccu.table_name AS foreign_table_name, ccu.column_name AS foreign_column_name FROM information_schema.table_constraints AS tc JOIN information_schema.key_column_usage AS kcu ON tc.constraint_name = kcu.constraint_name JOIN information_schema.constraint_column_usage AS ccu ON ccu.constraint_name = tc.constraint_name WHERE constraint_type = 'FOREIGN KEY' AND ccu.table_name='" +
            tablename + "'");
    query.exec();
    while (query.next()) {
        QPair<QString, QString> qp(query.value(5).toString(), query.value(6).toString());
